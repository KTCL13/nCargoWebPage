      ...
      <table role="grid" aria-label="Data table" className="w-full text-sm">
        <thead role="rowgroup">
          <tr role="row" className="border-b border-gray-100 bg-gray-50/50">
            <th role="columnheader" className="px-5 py-3 text-left font-subtitles text-xs uppercase tracking-wide text-gray-500">ID</th>
            <th role="columnheader" className="px-5 py-3 text-left font-subtitles text-xs uppercase tracking-wide text-gray-500">Tarea</th>
            <th role="columnheader" className="px-5 py-3 text-left font-subtitles text-xs uppercase tracking-wide text-gray-500">Asignado a</th>
            <th role="columnheader" className="px-5 py-3 text-left font-subtitles text-xs uppercase tracking-wide text-gray-500">Estado</th>
            <th role="columnheader" className="px-5 py-3 text-left font-subtitles text-xs uppercase tracking-wide text-gray-500">Vencimiento</th>
        ...
        <tbody role="rowgroup" className="divide-y divide-gray-50">
          {loading ? (
            <tr role="row"><td role="gridcell" colSpan={6} className="text-center py-12 text-gray-600 font-subtitles">Cargando tareas...</td></tr>
          ) : tasks.length === 0 ? (
            <tr role="row"><td role="gridcell" colSpan={6} className="text-center py-12 text-gray-600 font-subtitles">No hay tareas que coincidan con los filtros</td></tr>
          ) : tasks.map(task => (
            <tr role="row" key={task.id} className="hover:bg-gray-50 transition-colors">
              <td role="gridcell" className="px-5 py-4 font-mono text-xs text-gray-600">#{task.id}</td>

      ...
      <div className="bg-white rounded-[var(--radius-xl)] shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table role="grid" aria-label="Data table" className="w-full text-sm">
            <thead role="rowgroup"><tr role="row" className="border-b bg-gray-50"><th role="columnheader" className="px-4 py-3"><input type="checkbox" checked={contracts.length > 0 && selected.size === contracts.length} onChange={toggleSelectAll} aria-label="Seleccionar todos" /></th>{['Empleado', 'Cargo', 'Tipo', 'Salario', 'Vigente', 'Inicio', 'Fin', 'Acciones'].map(h => <th role="columnheader" key={h} className="px-4 py-3 text-left font-bold text-gray-500 uppercase text-[10px]">{h}</th>)}</tr></thead>
            <tbody role="rowgroup">
              {loading ? <tr role="row"><td role="gridcell" colSpan={10} className="text-center py-10 text-gray-600">Cargando...</td></tr> : contracts.length === 0 ? <tr role="row"><td role="gridcell" colSpan={10} className="text-center py-10 text-gray-600">Sin resultados</td></tr> : contracts.map(c => {
                const currentActive = dirty[c.id]?.isActive ?? c.isActive; const isDirty = dirty[c.id] !== undefined; const isSaving = saving.has(c.id)
                return (
                  <tr role="row" key={c.id} className="border-b hover:bg-gray-50/50 transition"><td role="gridcell" className="px-4 py-3"><input type="checkbox" checked={selected.has(c.id)} onChange={() => toggleSelect(c.id)} aria-label={`Seleccionar contrato ${c.id}`} /></td><td role="gridcell" className="px-4 py-3"><p className="font-bold">{c.employee.name}</p><p className="text-[10px] text-gray-600">{c.employee.email}</p></td><td role="gridcell" className="px-4 py-3">{c.job.title}</td><td role="gridcell" className="px-4 py-3 text-xs">{c.contractType?.name}</td><td role="gridcell" className="px-4 py-3 text-xs">{c.salary ? `$${c.salary}` : (c.hourlyRate ? `$${c.hourlyRate}/h` : '—')}</td><td role="gridcell" className="px-4 py-3"><button onClick={() => toggleActive(c.id, c.isActive)} aria-label={currentActive ? 'Desactivar contrato' : 'Activar contrato'} className={`w-8 h-4 rounded-full relative transition ${currentActive ? 'bg-green-500' : 'bg-gray-200'}`}><span className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition ${currentActive ? 'left-4.5' : 'left-0.5'}`} /></button></td><td role="gridcell" className="px-4 py-3 text-xs">{fmt(c.startDate)}</td><td role="gridcell" className="px-4 py-3 text-xs">{fmt(c.endDate)}</td><td role="gridcell" className="px-4 py-3 flex gap-2"><button onClick={() => saveRow(c.id)} disabled={!isDirty || isSaving} className={`text-xs px-3 py-1 rounded-lg font-bold ${isDirty && !isSaving ? 'bg-black text-white' : 'bg-gray-100 text-gray-600'}`}>{isSaving ? '...' : 'OK'}</button><button onClick={() => openHistory(c.employee.id, c.employee.name)} className="text-xs px-3 py-1 rounded-lg bg-blue-50 text-blue-600 font-bold">Log</button></td></tr>
                )
              })}
            </tbody>
          </table>

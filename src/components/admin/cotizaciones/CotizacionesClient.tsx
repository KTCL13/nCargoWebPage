        ...
        {tab === 'offices' && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden" role="tabpanel">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {offices.map(o => (
                  <tr key={o.id} className="hover:bg-gray-50"><td className="px-5 py-4 font-semibold">{o.name}</td><td className="px-5 py-4 text-gray-600 truncate max-w-[300px]">{o.address}</td><td className="px-5 py-4"><button onClick={() => toggleOfficeActive(o)} aria-label={o.isActive ? 'Desactivar almacén' : 'Activar almacén'} className={`text-xs font-bold px-2 py-1 rounded-full ${o.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{o.isActive ? 'Activo' : 'Inactivo'}</button></td><td className="px-5 py-4 text-right flex justify-end gap-2"><button aria-label="Editar almacén" onClick={() => openEditOffice(o)}>✏️</button><button aria-label="Eliminar almacén" onClick={() => deleteOffice(o.id)}>🗑️</button></td></tr>
                ))}
              </tbody>
            </table>
          </div>

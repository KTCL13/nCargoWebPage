    ...
    <DashboardLayout pageTitle="Dashboard" navItems={NAV_ITEMS} onReload={fetchAll}>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KPICard label="Tareas en proceso" value={inProgressTasks} icon="⚡" sub="Carga actual del equipo" subColor="text-orange-500" />
          <KPICard label="Jornadas registradas" value={todayAttendances} icon="⏱️" sub="Total en el sistema" subColor="text-green-500" />
          <KPICard label="Cotizaciones" value={totalQuotations} icon="📦" sub="Total registradas" subColor="text-green-500" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-[var(--radius-xl)] border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-gray-50 flex justify-between items-center"><h3 className="font-bold">Empleados Recientes</h3><button onClick={() => navigateTo('empleados')} className="text-blue-600 text-xs font-semibold hover:underline">Ver todos</button></div>
            <div className="p-2">
              {recentEmployees.length === 0 ? <p className="text-center text-gray-400 text-sm py-6">Sin datos</p> : recentEmployees.map(emp => (
                <EmployeeRow key={emp.id} name={emp.name} role={emp.activeContract?.job.title ?? emp.roles[0] ?? 'Sin cargo'} initials={initials(emp.name)} status={emp.status === 'ACTIVE' ? 'Activo' : 'Inactivo'} statusColor={emp.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'} />
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-[var(--radius-xl)] border border-gray-100 shadow-sm p-5 relative">
            ...
            {pieData.length === 0 ? <p className="text-center text-gray-400 text-sm py-6">Sin datos de rendimiento</p> : (
              <ResponsiveContainer width="100%" height={260} minWidth={0} minHeight={0} debounce={50}>
                <PieChart><Pie data={pieData} innerRadius={65} outerRadius={100} paddingAngle={4} dataKey="value">{pieData.map((entry, i) => <Cell key={i} fill={entry.color} />)}</Pie><Tooltip formatter={(v) => [`${v} tareas`, '']} /><Legend formatter={(value) => <span className="text-xs text-gray-600">{value}</span>} /></PieChart>
              </ResponsiveContainer>
            )}
          </div>
        </div>
...
function KPICard({ label, value, icon, sub, subColor, dark = false }: { label: string; value: number | null; icon: string; sub: string; subColor: string; dark?: boolean }) {
  return (
    <div className={`${dark ? 'bg-[var(--color-foreground)] text-white' : 'bg-white border-gray-100'} p-6 rounded-[var(--radius-xl)] shadow-sm relative overflow-hidden border`}>
      <div className="flex justify-between items-start mb-4"><span className={`${dark ? 'text-white/60' : 'text-gray-400'} text-xs font-bold uppercase tracking-wider`}>{label}</span><span className="text-2xl">{icon}</span></div>
...
function EmployeeRow({ name, role, initials, status, statusColor }: { name: string; role: string; initials: string; status: string; statusColor: string }) {
  return (
    <div className="flex items-center gap-4 p-3 hover:bg-gray-50 rounded-xl transition-colors">
      ...
      <div className="flex-1 min-w-0"><div className="text-sm font-bold text-gray-900 truncate">{name}</div><div className="text-xs text-gray-500 truncate">{role}</div></div>

  ...
  return (
    <DashboardLayout pageTitle="Reportes" navItems={NAV_ITEMS} onReload={handleReload}>
      <div className="space-y-6">
        ...
        <Section
          title="🚨 Alertas"
          badge={highAlerts > 0 ? `${highAlerts} prioridad alta` : undefined}
          actions={
            <SortSelect
              value={alertSort}
              onChange={v => { setAlertSort(v); setAlertPage(0) }}
              options={[
                { value: 'severity_asc', label: 'Severidad' },
          ...
          {loading.alerts ? <LoadingSkeleton rows={3} /> : sortedAlerts.length === 0 ? (
            ...
            <>
                ...
                {pagedAlerts.map((a, i) => {
                  ...
                  return (
                    <div key={i} className={`rounded-xl border p-4 transition-colors ${
                      isHigh ? 'border-red-200 bg-red-50/40'
                      : isMed ? 'border-amber-200 bg-amber-50/40'
                      : 'border-gray-200 bg-gray-50/60'
                    }`}>
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <span className="font-semibold text-sm text-gray-800">{a.employeeName}</span>
              ...
              <div className="mt-4 pt-4 border-t border-gray-50">
                <Pagination page={alertPage} pageSize={PAGE_SIZE} totalItems={sortedAlerts.length} onPageChange={setAlertPage} onPageSizeChange={() => {}} />
              </div>
            </>
          )}
        </Section>
        ...
        <Section
          ...
          actions={
            <SortSelect
              ...
              options={[
                { value: 'tasksCompleted_desc', label: 'Más tareas' },
                { value: 'totalWorkedHours_desc', label: 'Más horas' },
                { value: 'notDoneCount_desc', label: 'Más incumplimientos' },
              ]}
            />
          }
        >
          {loading.performance ? <LoadingSkeleton rows={4} /> : sortedPerformance.length === 0 ? (
            <EmptyState message="Sin datos de rendimiento." />
          ) : (
            <>
              <div className="space-y-3">
                {sortedPerformance.map(p => (
                  <div key={p.employeeId} className={`rounded-xl border p-4 transition-colors ${
                    p.notDoneCount > 0
                    ...
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <span className="font-semibold text-sm text-gray-800">{p.employeeName}</span>
                      <span className={`shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold ${
                        p.notDoneCount > 0
                          ? 'bg-red-100 text-red-700 border border-red-200'
                          : 'bg-emerald-100 text-emerald-700 border border-emerald-200'
                      }`}>
                        {p.notDoneCount > 0 ? `${p.notDoneCount} no ${p.notDoneCount === 1 ? 'hecha' : 'hechas'}` : '● Al día'}
                      </span>
                    </div>
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">
                        {p.tasksCompleted} completadas
                      </span>
                      <span className="text-gray-400">{p.totalWorkedHours}h trabajadas</span>
                      {p.avgCompletionMinutes !== null && (
                        <span className="font-mono text-gray-500">~{fmt(p.avgCompletionMinutes)} prom.</span>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <div className="mt-4 pt-4 border-t border-gray-50">
                <Pagination page={perfPage} pageSize={PERF_LIMIT} totalItems={perfTotal} onPageChange={handlePerfPageChange} onPageSizeChange={() => {}} />
              </div>
            </>
          )}
        </Section>
        ...
        <Section
          ...
          actions={
            <SortSelect
              ...
              options={[
                { value: 'totalTasks_desc', label: 'Total tareas' },
                { value: 'inProgressCount_desc', label: 'En proceso' },
                { value: 'employeeName_asc', label: 'Empleado A–Z' },
              ]}
            />
          }
        >
          {loading.workload ? <LoadingSkeleton rows={4} /> : sortedWorkload.length === 0 ? (
            <EmptyState message="Sin datos de carga." />
          ) : (
            <>
              <div className="space-y-3">
                {pagedWorkload.map(w => (
                  <div key={w.employeeId} className={`rounded-xl border p-4 transition-colors ${
                    w.notDoneCount > 0
                      ? 'border-red-200 bg-red-50/30'
                      : w.inProgressCount > 0
                    ...
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <span className="font-semibold text-sm text-gray-800">{w.employeeName}</span>
                      <span className="shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold bg-gray-100 text-gray-600 border border-gray-200">
                        {w.totalTasks} total
                      </span>
                    </div>
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
                      {w.pendingCount > 0 && (
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full font-medium bg-yellow-50 text-yellow-700 border border-yellow-200">
                          {w.pendingCount} pendientes
                        </span>
                      )}
                      {w.inProgressCount > 0 && (
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full font-medium bg-blue-50 text-blue-700 border border-blue-200">
                          {w.inProgressCount} en proceso
                        </span>
                      )}
                      {w.completedCount > 0 && (
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">
                          {w.completedCount} completadas
                        </span>
                      )}
                      {w.notDoneCount > 0 && (
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full font-medium bg-red-50 text-red-700 border border-red-200">
                          {w.notDoneCount} no hechas
                        </span>
                      )}
                    </div>
                  </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-50">
                <Pagination page={workloadPage} pageSize={PAGE_SIZE} totalItems={sortedWorkload.length} onPageChange={setWorkloadPage} onPageSizeChange={() => {}} />
              </div>
            </>
          )}
        </Section>
        ...
        <Section
          ...
          actions={
            <SortSelect
              ...
              options={[
                { value: 'minutesSpent_asc', label: 'Más rápidas' },
                { value: 'minutesSpent_desc', label: 'Más lentas' },
                { value: 'employeeName_asc', label: 'Empleado A–Z' },
              ]}
            />
          }
        >
          {loading.completion ? <LoadingSkeleton rows={4} /> : sortedCompletion.length === 0 ? (
            <EmptyState message="Sin tareas completadas en el período." />
          ) : (
            <>
              <div className="space-y-3">
                {pagedCompletion.map(c => (
                  <div key={c.taskId} className="rounded-xl border border-gray-200 bg-gray-50/60 p-4 transition-colors hover:border-gray-300">
                    <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500">
                      ...
                      {c.minutesSpent !== null && (
                        <span className="font-mono text-gray-500">{fmt(c.minutesSpent)}</span>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <div className="mt-4 pt-4 border-t border-gray-50">
                <Pagination page={completionPage} pageSize={PAGE_SIZE} totalItems={sortedCompletion.length} onPageChange={setCompletionPage} onPageSizeChange={() => {}} />
              </div>
            </>
          )}
        </Section>
    ...
    <div className="bg-white rounded-[var(--radius-xl)] border border-gray-100 shadow-sm p-5">
      <div className="flex items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-3 min-w-0">
          ...
          {badge && (
            <span className="shrink-0 px-2 py-0.5 rounded-full bg-red-100 text-red-600 text-xs font-bold">{badge}</span>
          )}
        </div>
...
function SortSelect({ value, onChange, options }: { value: string; onChange: (v: string) => void; options: { value: string; label: string }[] }) {
  return (
    <select
      ...
      className="text-xs border border-gray-200 rounded-md px-2 py-1 text-gray-600 bg-white hover:border-gray-300 focus:outline-none cursor-pointer"
    >
